    ...
    for pos, entidad, total_valor in zip(posiciones, entidades, suma_total):
        ...
        for i, col in enumerate(columnas_barras):
            ...
            if textos_personalizados_barra is not None and i < textos_personalizados_barra.shape[1]:
                ...
                if porce_abajo:
                    if area_barra >= area_min or opcion_area_min == 'sin etiquetas':
                        ...
                        # Guardar ambos textos para dibujarlos fuera
                        etiquetas_fuera.append({
                            'pos': pos, 'col': col, 'texto': texto_valor, 'color': color,
                            'text_color': text_color_valor, 'font_size': font_config['valor_porcentaje_barra']['size'],
                            'font_weight': font_config['valor_porcentaje_barra']['weight'], 'font_family': font_config['family'],
                            'order': i, 'bottom_pos': bar_bottom + current_val, 'is_porce_abajo': True, 'is_valor': True
                        })
                        etiquetas_fuera.append({
                            'pos': pos, 'col': col, 'texto': texto_porcentaje, 'color': color,
                            'text_color': text_color_porcentaje, 'font_size': font_config['valor_porcentaje_barra_porcentaje']['size'],
                            'font_weight': font_config['valor_porcentaje_barra_porcentaje']['weight'], 'font_family': font_config['family'],
                            'order': i, 'bottom_pos': bar_bottom + current_val, 'is_porce_abajo': True, 'is_valor': False
                        })
            ...
            if (porce_barra or valor_barra or (entidad in textos_barra_personalizados)):
                if texto_a_dibujar and texto_final: # Asegurarse de que solo se ejecute si texto_a_dibujar es True
                    if area_barra >= area_min or opcion_area_min == 'sin etiquetas':
    ...
    if opcion_area_min == 'fuera' and etiquetas_fuera:
        ...
        for pos, lista_etqs in etiquetas_por_barra.items():
            ...
            if positivas:
                ...
                # Manejar porce_abajo fuera de la barra
                porce_abajo_items = [p for p in positivas if p.get('is_porce_abajo')]
                otros_items = [p for p in positivas if not p.get('is_porce_abajo')]

                idx_offset = 0
                for idx, etq in enumerate(otros_items):
                    extra_spacing = spacing * (idx + idx_offset)
                    y = y_base + extra_spacing + separacion_etiqueta_barra
                    ax.text(
                        pos, y, etq['texto'], ha='center', va='bottom',
                        fontsize=etq['font_size'], fontweight=etq['font_weight'],
                        fontfamily=etq['font_family'], color=etq['color'],
                        bbox=dict(facecolor='white', edgecolor='none', alpha=0.0, boxstyle='round,pad=0.1')
                    )
                idx_offset += len(otros_items)

                if porce_abajo_items:
                    valor_item = next((p for p in porce_abajo_items if p.get('is_valor')), None)
                    porce_item = next((p for p in porce_abajo_items if not p.get('is_valor')), None)
                    if valor_item and porce_item:
                        extra_spacing = spacing * idx_offset
                        y = y_base + extra_spacing + separacion_etiqueta_barra
                        # Dibujar valor
                        ax.text(pos, y, valor_item['texto'], ha='center', va='bottom',
                                fontsize=valor_item['font_size'], fontweight=valor_item['font_weight'],
                                fontfamily=valor_item['font_family'], color=valor_item['color'])
                        # Dibujar porcentaje (ligeramente debajo del valor)
                        ax.text(pos, y, porce_item['texto'], ha='center', va='top',
                                fontsize=porce_item['font_size'], fontweight=porce_item['font_weight'],
                                fontfamily=porce_item['font_family'], color=porce_item['color'])
